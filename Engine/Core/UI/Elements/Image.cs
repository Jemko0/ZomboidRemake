...
namespace Iso.Engine.Core.UI.Elements
{
    ...
    public class Image : UIElement
    {
        ...
        private void RenderNineSlice(SpriteBatch sb, Rectangle dest)
        {
            ...
            // 1. SAFETY FALLBACK
            // If texture is too small for the requested slice size (e.g. 1x1 white pixel),
            // or if the slice size is 0, just draw a standard stretched image.
            ...
            // 2. DESTINATION CLAMPING
            // If the UI element is smaller than the corners (e.g. button is 10px, corners are 8px),
            // we shrink the corner size for rendering so they don't overlap/invert.
            int renderS_X = Math.Min(s, dest.Width / 2);
            int renderS_Y = Math.Min(s, dest.Height / 2);

            // 3. SOURCE RECTANGLES (The Texture)
        ...
        new Rectangle(texW - s, texH - s, s, s)        // 8: Bottom Right
            ...
            // 4. DESTINATION RECTANGLES (The Screen)
        ...
        new Rectangle(dest.Right - renderS_X, dest.Bottom - renderS_Y, renderS_X, renderS_Y)// 8: BR
            ...
            for (int i = 0; i < 9; i++)
            {
                ...
                // Ensure both source AND destination rectangles are valid
